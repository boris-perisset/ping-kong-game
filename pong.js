...
function draw() {
  ...
  /*
    function startText () {
      this.title = text("Click to Play", width/2, height/2),
      this.hover = cursor(HAND),
      this.action =  function mouseClicked() {
        state.game = true;
        state.start = false;
      }
    }
    startText ();
    console.log ("Start", state.start, "Game", state.game)
    */
  }
  ...
  if (state.over == true) {
    ...
    let finalMessages = [result, "3", "2", "1"];

    for (let message of finalMessages) {
      frameRate (1);
